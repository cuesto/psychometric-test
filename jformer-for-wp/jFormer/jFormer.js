...
function progressbarpertrans() {
    jQuery('html, body').animate({scrollTop: 0}, 500);
    return 100 / (countdownValues.length);
}

function displayTimer()
{
    if(timerDisplayed == false)
    {
        timerDisplayed = true;
            if (countdownSet==1) {
...
function hideTimer(){
    if(timerDisplayed == true)
    {
        timerDisplayed = false;
        jQuery(".timer").hide();
        jQuery(".jFormerTitle").hide();
    }
}

function countdownInit() {
    if(getParam('test_id')>5)
    {        var aa = document.getElementById("duration_timer").innerHTML;
        var aa = aa.split("$");
        countdownSet = 0;
        for (var i = 0; i < aa.length-1; i++) {
            countdownValues[i] = aa[i].split("&");
            if (countdownValues[0][0] > 0) {
                countdownSet = 1;
            }
        }
        
        if(countdownSet==1) {
            var textInsideContainer = "<div class=\"example exampleA\" style=\"float:right;\"><div class=\"timer\" style=\"zoom:0.5;\"></div></div>";
        }
        else if(countdownSet ==0) {
            var textInsideContainer = "<div class=\"example exampleA\" style=\"float:right;\"></div>";
        }
        ...
        progressbarval = progressbarpertrans();
...
function updateProgressbar(increment) {
    ...
    jQuery("#progress_percentage").html(progressbarval+'%');
}
...
function runWhenFinished() {
    for (var i = 0; i < (countdownValues.length-pagesPassed); i++) {
        jQuery('.nextButton').trigger("submit");
        updateProgressbar("1");
    }
}
...
DateInput = (function ($) {
    ...
    function DateInput(element, options) {
        ...
        jQuery.extend(this, DateInput.DEFAULT_OPTS, options);

        var button = jQuery('<span class="jFormComponentDateButton">Find Date</span>');

        this.input = jQuery(element);
        this.input.after(button);
        this.button = jQuery(element).parent().find('span.jFormComponentDateButton');
    ...
    DateInput.prototype = {
        build: function () {
            var monthNav = jQuery('<p class="jFormComponentDateSelectorMonthNavigator">' + '<span class="jFormComponentDateSelectorButton jFormComponentDateSelectorPrevious" title="[Page-Up]">&#171;</span>' + ' <span class="jFormComponentDateSelectorMonthName"></span> ' + '<span class="jFormComponentDateSelectorButton jFormComponentDateSelectorNext" title="[Page-Down]">&#187;</span>' + '</p>');
            this.monthNameSpan = jQuery(".jFormComponentDateSelectorMonthName", monthNav);
            jQuery(".jFormComponentDateSelectorPrevious", monthNav).click(this.bindToObj(function () {
                this.moveMonthBy(-1);
            }));
            jQuery(".jFormComponentDateSelectorNext", monthNav).click(this.bindToObj(function () {
                this.moveMonthBy(1);
            }));

            var yearNav = jQuery('<p class="jFormComponentDateSelectorYearNavigator">' + '<span class="jFormComponentDateSelectorButton jFormComponentDateSelectorPrevious" title="[Ctrl+Page-Up]">&#171;</span>' + ' <span class="jFormComponentDateSelectorYearName"></span> ' + '<span class="jFormComponentDateSelectorButton jFormComponentDateSelectorNext" title="[Ctrl+Page-Down]">&#187;</span>' + '</p>');
            this.yearNameSpan = jQuery(".jFormComponentDateSelectorYearName", yearNav);
            jQuery(".jFormComponentDateSelectorPrevious", yearNav).click(this.bindToObj(function () {
                this.moveMonthBy(-12);
            }));
            jQuery(".jFormComponentDateSelectorNext", yearNav).click(this.bindToObj(function () {
                this.moveMonthBy(12);
            }));

            var nav = jQuery('<div class="jFormComponentDateSelectorNavigator"></div>').append(monthNav, yearNav);
            ...
            this.dateSelector = this.rootLayers = jQuery('<div class="jFormComponentDateSelector"></div>').append(nav, tableShell).insertAfter(this.input);

            if (jQuery.browser.msie && jQuery.browser.version < 7) {

                this.ieframe = jQuery('<iframe class="jFormComponentDateSelectorIEFrame" frameborder="0" src="#"></iframe>').insertBefore(this.dateSelector);
                this.rootLayers = this.rootLayers.add(this.ieframe);

                jQuery(".jFormComponentDateSelectorButton", nav).mouseover(function () {
                    jQuery(this).addClass("hover")
                });
                jQuery(".jFormComponentDateSelectorButton", nav).mouseout(function () {
                    jQuery(this).removeClass("hover")
                });
            };

            this.tbody = jQuery("tbody", this.dateSelector);
        ...
        selectMonth: function (date) {
            ...
            if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() && this.currentMonth.getMonth() == newMonth.getMonth())) {
                ...
                jQuery(".jFormComponentDateSelectorSelectedDay", this.tbody).click(this.bindToObj(function (event) {
                    this.changeInput(jQuery(event.target).attr("date"));
                }));

                jQuery("td[date=" + this.dateToString(new Date()) + "]", this.tbody).addClass("jFormComponentDateSelectorToday");

                jQuery("td.jFormComponentDateSelectorSelectedDay", this.tbody).mouseover(function () {
                    jQuery(this).addClass("hover")
                });
                jQuery("td.jFormComponentDateSelectorSelectedDay", this.tbody).mouseout(function () {
                    jQuery(this).removeClass("hover")
                });
            };

            jQuery('.jFormComponentDateSelectorSelected', this.tbody).removeClass("jFormComponentDateSelectorSelected");
            jQuery('td[date=' + this.selectedDateString + ']', this.tbody).addClass("jFormComponentDateSelectorSelected");
        },
        ...
        show: function () {
            ...
            jQuery(document.body).keydown(this.keydownHandler);
        ...
        hide: function () {
            ...
            jQuery([window, document.body]).unbind("click", this.hideIfClickOutside);
            this.button.click(this.show);
            this.input.focus(this.show);
            jQuery(document.body).unbind("keydown", this.keydownHandler);
        },
        ...
        setPosition: function () {
            ...
            if (top + jQuery(window).height() > bottom) {} else {
                jQuery.scrollTo(bottom - jQuery(window).height() + 'px', 250, {
                    axis: 'y'
                });
            }
        },
    ...
    jQuery.fn.date_input = function (opts) {
    ...
    jQuery.date_input = {
        initialize: function (opts) {
            jQuery("input.date_input").date_input(opts);
        }
    };
...
*/
(function ($) {
    var pasteEventName = (jQuery.browser.msie ? 'paste' : 'input') + ".mask";
    ...
    jQuery.fn.extend({
        ...
        mask: function (mask, settings) {
            if (!mask && this.length > 0) {
                ...
                return jQuery.map(input.data("buffer"), function (c, i) {
            ...
            var defs = jQuery.mask.definitions;
            var tests = [];
            ...
            return this.each(function () {
                ...
                var buffer = jQuery.map(mask.split(""), function (c, i) {
                ...
                function keydownEvent(e) {
                    var pos = jQuery(this).caret();
                    var k = e.keyCode;
                    ignore = (k < 16 || (k > 16 && k < 32) || (k > 32 && k < 41));
                    if (!e.shiftKey) {
                        if (k == 36) {
                            e.preventDefault();
                            jQuery(this).caret(seekNext(0));
                        }
 ...
 */;
(function ($) {
    var m = jQuery.scrollTo = function (b, h, f) {
            jQuery(window).scrollTo(b, h, f)
        };
    m.defaults = {
        axis: 'xy',
    ...
    jQuery.fn.scrollable = function () {
        return this.map(function () {
            var b = this,
                h = !b.nodeName || jQuery.inArray(b.nodeName.toLowerCase(), ['iframe', '#document', 'html', 'body']) != -1;
            if (!h) return b;
            var f = (b.contentWindow || b).document || b.ownerDocument || b;
            return jQuery.browser.safari || f.compatMode == 'BackCompat' ? f.body : f.documentElement
        })
    };
    jQuery.fn.scrollTo = function (l, j, a) {
        if (typeof j == 'object') {
            a = j;
            j = 0
        }
        ...
        return this.scrollable().each(function () {
            ...
            case 'string':
                ...
                d = jQuery(d, this);
            case 'object':
                if (d.is || d.style) p = (d = jQuery(d)).offset()
            }
            jQuery.each(a.axis.split(''), function (b, h) {
                var f = h == 'x' ? 'Left' : 'Top',
                    i = f.toLowerCase(),
                    c = 'scroll' + f,
                    r = k[c],
...
(function ($) {
    ...
    jQuery.fn[a] = function (y) {
        y = jQuery.extend({}, C.defaults, y);
        var z = y.event,
            A = y.step,
            B = y.lazy;
        return this.each(function () {
            var j = y.target ? this : document,
                k = jQuery(y.target || this, j),
                l = k[0],
                m = y.items,
                o = y.start,
                p = y.interval,
            ...
            if (!B) m = w();
            if (y.force) t({}, o);
            jQuery(y.prev || [], j)[c](z, -A, s);
            jQuery(y.next || [], j)[c](z, A, s);
            ...
            if (q) q = jQuery(q, j)[c](z, function (e) {
 ...
 */ (function ($) {
    ...
    function Simpletip(elem, conf) {
        ...
        var tooltip = jQuery(conf.content).addClass(conf.baseClass).addClass((conf.fixed) ? conf.fixedClass : '').addClass((conf.persistent) ? conf.persistentClass : '').html(wrappedContent);
...
JFormer = Class.extend({
    init: function (formId, options) {
        ...
        // Class variables
        this.id = formId;
        this.form = jQuery(['form#', this.id].join(''));
        this.formData = {};
...
//            alert("5");
    ...
    initPages: function (jFormPages) {
        ...
        each(jFormPages, function (jFormPageKey, jFormPageValue) {
            ...
            if (jFormPage.options.dependencyOptions !== null) {
                jQuery.each(jFormPage.options.dependencyOptions.dependentOn, function (index, componentId) {
            ...
            each(jFormPageValue.jFormSections, function (jFormSectionKey, jFormSectionValue) {
                ...
                if (jFormSection.options.dependencyOptions !== null) {
                    jQuery.each(jFormSection.options.dependencyOptions.dependentOn, function (index, componentId) {
                        if (dependencies[componentId] === undefined) {
                            dependencies[componentId] = {
                                pages: [],
                                sections: [],
                ...
                each(jFormSectionValue.jFormComponents, function (jFormComponentKey, jFormComponentValue) {
                    ...
                    if (jFormComponent.options.dependencyOptions !== null) {
                        jQuery.each(jFormComponent.options.dependencyOptions.dependentOn, function (index, componentId) {
                            if (dependencies[componentId] === undefined) {
                                dependencies[componentId] = {
                                    pages: [],
                                    sections: [],
        ...
        jQuery.each(dependencies, function (componentId, dependentTypes) {

            jQuery('#' + componentId + ':text, textarea#' + componentId).bind('keyup', function (event) {
                jQuery.each(dependentTypes.pages, function (index, object) {
                    self.jFormPages[object.jFormPageId].checkDependencies();
                });
                jQuery.each(dependentTypes.sections, function (index, object) {
                    self.jFormPages[object.jFormPageId].jFormSections[object.jFormSectionId].checkDependencies();
                });
                jQuery.each(dependentTypes.components, function (index, object) {
            ...
            jQuery('#' + componentId + '-wrapper').bind('jFormComponent:changed', function (event) {
    ...
    select: function (jFormComponentId) {
        var componentFound = false,
            component = null;
        jQuery.each(this.jFormPages, function (jFormPageKey, jFormPage) {
            jQuery.each(jFormPage.jFormSections, function (sectionKey, sectionObject) {
                jQuery.each(sectionObject.jFormComponents, function (componentKey, componentObject) {
    ...
    checkDependencies: function (onInit) {
        jQuery.each(this.jFormPages, function (jFormPageKey, jFormPage) {
            ...
            jQuery.each(jFormPage.jFormSections, function (jFormSectionKey, jFormSection) {
                jFormSection.checkDependencies();

                jQuery.each(jFormSection.jFormComponents, function (jFormComponentKey, jFormComponent) {
    ...
    addSaveStateToSplashPage: function () {
        ...
        jQuery.each(self.options.saveState.components, function (jFormComponentId, jFormComponentOptions) {
        ...
        saveStateJFormComponents.saveStateStatus.component.find('input:option').bind('click', {
            context: this
        }, function (event) {
            // Remove any failure notices
            self.form.find('li.jFormerFailureNotice').remove();

            formState = jQuery(event.target).val();
        ...
        self.options.splashPage.controlSplashButton.bind('click', {
            context: this
        }, function (event) {
            event.preventDefault();
                    ...
                    success: function (json) {
                        ...
                        if (json.status == 'success') {
                            if (formState == 'newForm') {
                                ...
                                if (self.jFormPages[json.response.meta.currentPage].active === false) {
                                    self.currentJFormPageIdArrayIndex = jQuery.inArray(json.response.meta.currentPage, self.jFormPageIdArray);
                        ...
                        // If the form already exists
                            ...
                            if (json.response.failureNoticeHtml) {
                                self.control.append(jQuery('<li class="jFormerFailureNotice jFormComponentValidationFailed">' + json.response.failureNoticeHtml + '</li>'));
                        ...
                        // If the request failed
                            ...
                            if (json.response.failureNoticeHtml) {
                                self.control.append(jQuery(['<li class="jFormerFailureNotice jFormComponentValidationFailed">', json.response.failureNoticeHtml, '</li>'].join('')));
    ...
    updatePageNavigator: function () {
        ...
        for (var i = 1; i <= this.maxJFormPageIdArrayIndexReached + 1; i++) {
            pageCount = i;
            var jFormPageNavigatorLink = jQuery('#navigatePage' + pageCount);
    ...
    renumberPageNavigator: function () {
        jQuery('.jFormPageNavigatorLink:visible').each(function (index, element) {
            // Renumber page link icons
            if (jQuery(element).find('span').length > 0) {
                jQuery(element).find('span').html(index + 1);
            }
            // Relabel pages that have no title or icons
            else {
                jQuery(element).html('Page ' + (index + 1));
            }
        });
    },
    ...
    removeJFormPage: function (jFormPageId) {
        ...
        // Remove the HTML
        jQuery('#' + jFormPageId).remove();

        this.jFormPageIdArray = jQuery.grep(self.jFormPageIdArray, function (value) {
...
//                    alert("2f");
    ...
    countdownChangeSection: function () {
 //       alert("q");
        /*var self = this;
    ...
    countdownInit: function () {
        ...
        for (var i = 0; i < aa.length; i++) {
            this.countdownValues[i] = aa[i].split("&");
            if (this.countdownValues[0][0] > 0) {
                this.countdownSet = 1;
            }
        }
        jQuery('.exampleA .timer').spriteTimer({
            'seconds': 10,
            'isCountDown': true,
            'digitImagePath': 'timer/numbers.png',
            'callback': runWhenFinished
        });
        //                alert(this.countdownValues);
    },

    resetCoundown: function () {
        //    this.countdownChangeSection();
        //    alert(countdownPage);
        this.countdownPage++;
        //remove the previous button
    ...
    cRT: function () {
        var self = this;
        alert(bbb); //self.submitEvent(event);
        return "1";
    },

    runWhenFinished: function () {
        var self = this;
        //jQuery('.exampleA').css({'background':'#333'});
        //var self = this;
        //self.submitEvent(event);
        self.jFormer.cRT();
        //alert(this);
...
//            alert("4");
    ...
    initSaveState: function () {
        var self = this,
            interval = this.options.saveState.interval * 1000;
    ...
    getData: function () {
        var self = this;
        this.formData = {};
        jQuery.each(this.jFormPages, function (jFormKey, jFormPage) {
...
//            alert("6");
    ...
    scrollToPage: function (jFormPageId, options) {
        ...
        self.currentJFormPage.page, self.options.animationOptions.pageScroll.duration, {
            onAfter: function () {
                // Don't hide any pages while scrolling
                if (jQuery(self.jFormPageWrapper).queue('fx').length <= 1) {
    ...
    scrollToTop: function () {
        ...
        if (jQuery(window).scrollTop() > this.form.offset().top) {
            jQuery(document).scrollTo(self.form, self.options.animationOptions.pageScroll.duration, {
                offset: {
                    top: -10
                }
            });
    ...
    getTimeActive: function () {
        var currentTotal = 0;
        jQuery.each(this.jFormPages, function (key, page) {
...
//                alert("8");
    ...
    submitForm: function (event) {
        ...
        formClone.empty();
        formClone.appendTo(jQuery(this.form).parent());
        // Wrap all of the form responses into an object based on the component jFormComponentType
        var formData = jQuery('<input type="hidden" name="jFormer" />').attr('value', encodeURI(jFormerUtility.jsonEncode(this.getData()))); // Set all non-file values in one form object
        var formIdentifier = jQuery('<input type="hidden" name="jFormerId" value="' + this.id + '" />');
        var formIdentifier123 = jQuery('<input type="hidden" name="jFormerId123" value="' + getParam('test_id') + '" />');
        formClone.append(formData);
        formClone.append(formIdentifier);
        formClone.append(formIdentifier123);

        var formExtra = jQuery('<input type="hidden" name="action" value="jFormerForWp" />');
        ...
        this.form.find('input:file').each(function (index, fileInput) {
            if (jQuery(fileInput).val() != '') {
                ...
                var sectionId = jQuery(fileInput).closest('.jFormSection').attr('id');
                var pageId = jQuery(fileInput).closest('.jFormPage').attr('id');
                //var fileInput = jQuery(fileInput).clone()

                // do find out the section instance index
                if (jQuery(fileInput).attr('id').match(/-section[0-9]+/)) {
                    var sectionInstance = null;
                    var section = jQuery(fileInput).closest('.jFormSection');
                    ...
                    section.closest('.jFormPage').find('div[id*=' + sectionBaseId + ']').each(function (index, fileSection) {
                        if (section.attr('id') == jQuery(fileSection).attr('id')) {
                ...
                if (jQuery(fileInput).attr('id').match(/-instance[0-9]+/)) {
                    // grab the base id of the component to find all sister components
                    var baseId = jQuery(fileInput).attr('id').replace(/-instance[0-9]+/, '')
                    var instance = null;
                    // Find out which instance it is
                    jQuery(fileInput).closest('.jFormSection').find('input[id*=' + baseId + ']').each(function (index, fileComponent) {
                        if (jQuery(fileComponent).attr('id') == jQuery(fileInput).attr('id')) {
                            instance = index + 1;
                            return false;
                        }
                        return true;
                    });
                    fileInput.attr('name', jQuery(fileInput).attr('name').replace(/-instance[0-9]+/, '-instance' + instance));
                }

                jQuery(fileInput).attr('name', jQuery(fileInput).attr('name') + ':' + pageId + ':' + sectionId);
                jQuery(fileInput).appendTo(formClone);
            }
        });
        ...
        this.form.find('input:file').each(function(index, fileInput) {
            if(jQuery(fileInput).val() != '') {
                ...
                var sectionId = jQuery(fileInput).closest('.jFormSection').attr('id');
                var pageId = jQuery(fileInput).closest('.jFormPage').attr('id');
                var clone = jQuery(fileInput).clone()
    ...
    handleFormSubmissionResponse: function (json) {
        ...
        if (json.status == 'failure') {
            ...
            if (json.response.validationFailed) {
                jQuery.each(json.response.validationFailed, function (jFormPageKey, jFormPageValues) {
                    jQuery.each(jFormPageValues, function (jFormSectionKey, jFormSectionValues) {
                        // Handle section instances
                        if (jQuery.isArray(jFormSectionValues)) {
                            jQuery.each(jFormSectionValues, function (jFormSectionInstanceIndex, jFormSectionInstanceValues) {
                                var sectionKey;
                                ...
                                jQuery.each(jFormSectionInstanceValues, function (jFormComponentKey, jFormComponentErrors) {
                        ...
                        // There are no section instances
                            ...
                            jQuery.each(jFormSectionValues, function (jFormComponentKey, jFormComponentErrors) {
        ...
        // Form passed processing
            ...
            if (json.response.successPageHtml) {
                ...
                var successPageDiv = jQuery('<div id="' + this.id + 'jFormPageSuccess" class="jFormPage jFormPageSuccess">' + json.response.successPageHtml + '</div>');
            ...
            // Show a failure page that allows you to go back
                ...
                var failurePageDiv = jQuery('<div id="' + this.id + 'jFormPageFailure" class="jFormPage jFormPageFailure">' + json.response.failurePageHtml + '</div>');
                ...
                this.control.append(jQuery('<li class="startOver"><button class="startOverButton">Start Over</button></li>'));
    ...
    showAlert: function (message, jFormComponentType, modal, options) {
        ...
        var animationOptions = jQuery.extend(this.options.animationOptions.alert, options);
    ...
    showModal: function (header, content, className, options) {
        ...
        var animationOptions = jQuery.extend(this.options.animationOptions.modal, options);

        // If there is no modal wrapper, add it
        if (modalWrapper.length == 0) {
            var modalTransparency = jQuery('<div class="jFormerModalTransparency"></div>');
            modalWrapper = jQuery('<div style="display: none;" class="jFormerModalWrapper"><div class="jFormerModal"><div class="jFormerModalHeader">' + header + '</div><div class="jFormerModalContent">' + content + '</div><div class="jFormerModalFooter"><button>Okay</button></div></div></div>');
            ...
            modalWrapper.find('button').click(function (event) {
                jQuery('.jFormerModalWrapper').hide(animationOptions.hideDuration);
                jQuery('.jFormerModalTransparency').hide(animationOptions.hideDuration);
                jQuery('.jFormerModalWrapper').remove();
                jQuery('.jFormerModalTransparency').remove();
        ...
        varWindow.resize(function (event) {
            ...
            jQuery('.jFormerModalTransparency').width(varWindow.width()).height(varWindow.height());
        });

        // If they click away from the modal (on the modal wrapper), remove it
        jQuery('.jFormerModalTransparency').click(function (event) {
            if (jQuery(event.target).is('.jFormerModalTransparency')) {
        ...
        jQuery('.jFormerModalTransparency').width(varWindow.width()).height(varWindow.height() * 1.1).css('top', varWindow.scrollTop());
        modal.css({
            'top': topMargin,
            'left': leftMargin
        });
    ...
    recordAnalytics: function () {
        ...
        if (!this.options.disableAnalytics) {
            setTimeout(function () {
                ...
                var image = jQuery('<img src="' + jsProtocol + 'jformer.com/analytics/analytics.gif?pageCount=' + self.jFormPageIdArray.length + '&componentCount=' + self.jFormComponentCount + '&formId=' + self.id + '" style="display: none;" />');
    ...
    updateProgressBar: function () {
        ...
        jQuery.each(this.jFormPages, function (pageKey, pageObject) {
            jQuery.each(pageObject.jFormSections, function (sectionKey, sectionObject) {
                jQuery.each(sectionObject.jFormComponents, function (componentKey, componentObject) {
        ...
        }, 500).html('<p>' + percentCompleted + '%</p>');
    },

    addBlurTipListener: function () {
        var self = this;
        jQuery(document).bind('blurTip', function (event, tipElement, action) {
            if (action == 'hide') {
                self.blurredTips = jQuery.map(self.blurredTips, function (tip, index) {
                    if (jQuery(tip).attr('id') == tipElement.attr('id')) {
                        return null
                    } else {
                        return tip;
                    }
                ...
                if (self.blurredTips.length > 0) {
                    jQuery.each(self.blurredTips, function (index, tip) {
                        jQuery(tip).addClass('jFormerTipBlurred')
                    })
                }
...
jQuery.extend(JFormerUtility.prototype, {
    isSet: function () {
        var a = arguments;
        var l = a.length;
        var i = 0;
    ...
    empty: function (mixedVariable) {
        var key;
        if (mixedVariable === "" || mixedVariable === 0 || mixedVariable === "0" || mixedVariable === null || mixedVariable === false || mixedVariable === undefined) {
            return true;
        }
...
JFormPage = Class.extend({
    init: function (jFormer, pageId, options) {
        ...
        if (this.options.onScrollTo.onBefore !== null) {
            var onBeforeFunction = jQuery.trim(this.options.onScrollTo.onBefore);
            this.options.onScrollTo.onBefore = function () {
                eval(onBeforeFunction);
            };
        }
        if (this.options.onScrollTo.onAfter !== null) {
            var onAfterFunction = jQuery.trim(this.options.onScrollTo.onAfter);
            this.options.onScrollTo.onAfter = function () {
                eval(onAfterFunction);
            };
        }
    ...
    getData: function () {
        ...
        if (this.disabledByDependency) {
            ...
            this.formData = {};
            jQuery.each(this.jFormSections, function (jFormSectionKey, jFormSection) {
    ...
    validate: function (silent) {
        ...
        if (self.validationPassed) {
            jQuery('#navigatePage' + (self.jFormer.currentJFormPageIdArrayIndex + 1)).removeClass('jFormPageNavigatorLinkWarning');
        } else if (!silent) {
    ...
    clearValidation: function () {
        jQuery.each(this.jFormSections, function (sectionKey, section) {
                ...
                each(sectionInstance.jFormComponents, function (componentLabel, component) {
                    each(component.instanceArray, function (instanceLabel, instance) {
                        if (!instance.validationPassed || instance.errorMessageArray.length > 0) {
                            ...
                            if (top < offset && top + jQuery(window).height() > instance.component.position().top) {
                                instance.component.find(':input:first').focus();
                                //instance.highlight();
                            } else {
                                jQuery.scrollTo(offset + 'px', 500, {
                                    onAfter: function () {
                                        instance.component.find(':input:first').focus();
                                        //instance.highlight();
                                    }
    ...
    disableByDependency: function (disable) {
        ...
        if (this.disabledByDependency !== disable) {
            var pageIndex = jQuery.inArray(this.id, this.jFormer.jFormPageIdArray);
            ...
            if (disable === true) {
                ...
                if (this.jFormer.options.pageNavigator !== false) {
                    ...
                    if (this.options.dependencyOptions.display == 'hide') {
                        jQuery('#navigatePage' + (pageIndex + 1)).hide();
                    ...
                    // Lock the page link
                        ...
                        jQuery('#navigatePage' + (pageIndex + 1)).addClass('jFormPageNavigatorLinkDependencyLocked').find('span').html('&nbsp;');
                    }
                }
            }
            // Show the page
                ...
                if (this.jFormer.options.pageNavigator !== false) {
                    // Show the page link
                    if (this.options.dependencyOptions.display == 'hide') {
                        jQuery('#navigatePage' + (pageIndex + 1)).show();
                    }
                    // Unlock the page link
                    else {
                        jQuery('#navigatePage' + (pageIndex + 1)).removeClass('jFormPageNavigatorLinkDependencyLocked');
                    }
...
JFormSection = Class.extend({
    init: function (parentJFormPage, sectionId, options) {
        this.options = jQuery.extend({
            dependencyOptions: null,
            // options {jsFunction:#, dependentOn:array, display:enum('hide','lock')}
    ...
    addSectionInstance: function () {
        ...
        if (this.instanceArray.length < this.options.instanceOptions.max || this.options.instanceOptions.max === 0) {
            ...
            if (this.options.instanceOptions.animationOptions !== undefined) {
                jQuery.extend(animationOptions, this.parentJFormPage.jFormer.options.animationOptions.instance, this.options.instanceOptions.animationOptions);
            } else {
                animationOptions = this.parentJFormPage.jFormer.options.animationOptions.instance;
            }
            //console.log(animationOptions);
            jQuery(instanceClone).append(removeButton);
            instanceClone.find('#' + buttonId).bind('click', function (event) {
                var target = jQuery(event.target);
                event.preventDefault();
                parent.instanceArray = jQuery.map(parent.instanceArray, function (cloneId, index) {
    ...
    nameSectionInstance: function (component) {
        var self = this,
            ending = '';
        jQuery(component).attr('id', jQuery(component).attr('id') + '-section' + this.iterations);
        jQuery(component).find('*').each(function (key, child) {
            if (jQuery(child).attr('id')) {
                changeName(child, 'id');
            }
            if (jQuery(child).attr('for')) {
        ...
        function changeName(child, attribute) {
            ending = getEnding(jQuery(child).attr(attribute));
            if (ending == '') {
                jQuery(child).attr(attribute, jQuery(child).attr(attribute) + '-section' + self.iterations + ending);
            } else {
                jQuery(child).attr(attribute, jQuery(child).attr(attribute).replace(ending, '-section' + self.iterations + ending));
            }
        }
    ...
    createSectionInstanceObject: function (instanceClone, options) {
        var tempOptions = jQuery.extend(true, {}, options);
        ...
        jQuery.each(this.jFormComponents, function (key, component) {
            var componentTempOptions = jQuery.extend(true, {}, component.options);
    ...
    relabelSectionInstances: function (instanceArray, animationOptions) {
        jQuery.each(instanceArray, function (key, instance) {
            if (key !== 0) {
                var count = key + 1,
    ...
    clearValidation: function () {
        jQuery.each(this.jFormComponents, function (componentKey, component) {
    ...
    getData: function () {
        ...
        if (this.disabledByDependency) {
            ...
            if (this.instanceArray.length > 1) {
                this.formData = [];
                jQuery.each(this.instanceArray, function (instanceIndex, instanceJFormSection) {
                    var sectionData = {};
                    jQuery.each(instanceJFormSection.jFormComponents, function (jFormComponentKey, jFormComponent) {
    ...
    setData: function (data) {
        ...
        if (jQuery.isArray(data)) {
            jQuery.each(data, function (index, instance) {
                if (index !== 0 && self.instanceArray[index] == undefined) {
                    self.addSectionInstance();
                }
                jQuery.each(instance, function (key, componentData) {
                    if (index !== 0) {
                ...
                /*jQuery.each(self.instanceArray[index].jFormComponents, function(key, component){

                   component.setData(instance[key]);
               });*/
            });
        } else {
            jQuery.each(data, function (key, componentData) {
    ...
    disableByDependency: function (disable) {
        ...
        if (self.parentJFormPage.jFormer.initializing) {
            ...
            animationOptions = jQuery.extend(animationOptions, this.parentJFormPage.jFormer.options.animationOptions.dependency, this.options.dependencyOptions.animationOptions);
        } else {
            animationOptions = this.parentJFormPage.jFormer.options.animationOptions.dependency;
        }

        var elementsToDisable = this.section;
        jQuery.each(this.instanceArray, function (index, sectionInstance) {
            if (index !== 0) {
        ...
        if (this.options.instanceOptions !== null && (this.instanceArray.length < this.options.instanceOptions.max || this.options.instanceOptions.max === 0)) {
            var addButton = jQuery(this.parentJFormSection.section.find('#' + this.id + '-addInstance'));
            if (self.parentJFormPage.jFormer.initializing) {
    ...
    checkChildrenDependencies: function () {
        jQuery.each(this.jFormComponents, function (jFormComponentKey, jFormComponent) {
            jFormComponent.checkDependencies();
        });
    }
});
...
JFormComponent = Class.extend({
    init: function (parentJFormSection, jFormComponentId, jFormComponentType, options) {
        this.options = jQuery.extend({
            validationOptions: [],
            // 'required', 'email', etc... - An array of validation keys used by this.validate() and jFormerValidator
            showErrorTipOnce: false,
            triggerFunction: null,
            // set to a function name, is a function
            componentChangedOptions: null,
            // set options for when component:changed is run
            dependencyOptions: null,
            // options {jsFunction:Javascript, dependentOn:array, display:enum('hide','lock')}
            instanceOptions: null,
            // options {max:#, addButtonText:string, removeButtonText:string}
            tipTargetPosition: 'rightMiddle',
            // 'rightMiddle' - Where the tooltip will be placed in relation to the component
            tipCornerPosition: 'leftTop',
            // 'leftTop' - The corner of the tip that will point to the tip target position
            isInstance: false
        }, options || {});
        ...
        this.component = jQuery('#' + jFormComponentId + '-wrapper');
        this.formData = null; // Will be an object is there is just one instance, will be an array if there is more than one instance
        ...
        if (jQuery.trim(this.tipDiv.html()) !== '') {
            this.addTip();
        }
    ...
    reformValidations: function () {
        var reformedValidations = {},
            self = this;
        jQuery.each(this.options.validationOptions, function (validationFunction, validationOptions) {
    ...
    defineComponentChangedEventListener: function () {
        ...
        this.component.find('input:checkbox, input:radio').each(function (key, input) {
            jQuery(input).bind('click', function (event) {
                jQuery(this).trigger('jFormComponent:changed', self);
            });
        });

        this.component.find(':input:not(button, :checkbox, :radio)').each(function (key, input) {
            jQuery(input).bind('change', function (event) {
                jQuery(this).trigger('jFormComponent:changed', self);
            });
        });
    },
    ...
    getData: function () {
        ...
        if (this.disabledByDependency || this.parentJFormSection.disabledByDependency) {
            ...
            if (this.instanceArray.length > 1) {
                this.formData = [];
                jQuery.each(this.instanceArray, function (index, component) {
    ...
    setData: function (data) {
        ...
        if (jQuery.isArray(data)) {
            jQuery.each(data, function (index, value) {
                if ((self.type == 'JFormComponentMultipleChoice' && (jQuery.isArray(value) || self.multipeChoiceType == 'radio')) || self.type != 'JFormComponentMultipleChoice') {
                    if (index !== 0 && self.instanceArray[index] == undefined) {
                        self.addInstance();
                    }
    ...
    createInstanceButton: function () {
        ...
        if (this.options.instanceOptions != null) {
            ...
            var addButton = jQuery('<button id="' + this.id + '-addInstance" class="jFormComponentAddInstanceButton">' + this.options.instanceOptions.addButtonText + '</button>');
    ...
    addInstance: function () {
        ...
        if (this.instanceArray.length < this.options.instanceOptions.max || this.options.instanceOptions.max === 0) {
            ...
            var animationOptions = {};
            if (this.options.instanceOptions.animationOptions !== undefined) {
                animationOptions = jQuery.extend(animationOptions, this.parentJFormSection.parentJFormPage.jFormer.options.animationOptions.instance, this.options.instanceOptions.animationOptions);
            } else {
                animationOptions = this.parentJFormSection.parentJFormPage.jFormer.options.animationOptions.instance;
            }

            // Create the remove button
            jQuery(instanceClone).append('<button id="' + this.id + '-removeInstance" class="jFormComponentRemoveInstanceButton">' + this.options.instanceOptions.removeButtonText + '</button>');
            ...
            if (this.options.dependencies != undefined) {
                ...
                instanceObject.component.find(':text, textarea').bind('keyup', function (event) {
                    jQuery.each(parent.options.dependencies.pages, function (index, object) {
                        objectTop.jFormPages[object.jFormPageId].checkDependencies();
                    });
                    jQuery.each(parent.options.dependencies.sections, function (index, object) {
                        objectTop.jFormPages[object.jFormPageId].jFormSections[object.jFormSectionId].checkDependencies();
                    });
                    jQuery.each(parent.options.dependencies.components, function (index, object) {
    ...
    nameInstance: function (component) {
        ...
        function changeName(child, attribute) {
            ending = getEnding(jQuery(child).attr(attribute));
            if (ending == '') {
                jQuery(child).attr(attribute, jQuery(child).attr(attribute) + '-instance' + self.iterations + ending);
            } else {
                jQuery(child).attr(attribute, jQuery(child).attr(attribute).replace(ending, '-instance' + self.iterations + ending));
            }
        }
        ...
        if (typeof (this.tip) !== 'function') {
            ...
            var tip = this.tipTarget.simpletip({
                ...
                onShow: function () {
                    ...
                    if (jQuery(window).scrollTop() + height < offset) {
                        jQuery.scrollTo(offset - height + 'px', 250, {
                            axis: 'y'
                        });
                    }
                }
    ...
    addTipListeners: function () {
        ...
        this.component.bind('jFormComponent:showTip', function (event) {
            ...
            if (self.tip && typeof (self.tip) == 'object' && jQuery.trim(self.tipDiv.html()) !== '') {
                self.tip.show();
            }
    ...
    clearValidation: function () {
        ...
        if (this.tip && typeof (this.tip) == 'object') {
            ...
            if (jQuery.trim(this.tipDiv.find('.tipContent').html()) == '') {
                this.tipDiv.hide();
            }
        }
    },
    ...
    validate: function (silent) {
        ...
        jQuery.each(this.options.validationOptions, function (validationType, validationOptions) {
            ...
            if (validation == 'success') {
                ...
                if (validationType.match('required')) {
                    ...
                    if (self.parentJFormSection.parentJFormPage.jFormer.options.pageNavigator != false) {
                        var pageIndex = jQuery.inArray(self.parentJFormSection.parentJFormPage.id, self.parentJFormSection.parentJFormPage.jFormer.jFormPageIdArray);
                        jQuery('#navigatePage' + (pageIndex + 1)).addClass('jFormPageNavigatorLinkWarning');
                    }
                }
                if (silent) {
                    silentValidationPassed = false;
                } else {
                    jQuery.merge(self.errorMessageArray, validation);
                }
            }
        });
        if (silent) {
    ...
    handleServerValidationResponse: function (errorMessageArray) {
        ...
        jQuery.each(this.instanceArray, function (instanceKey, instance) {
            ...
            if (this.instanceArray.length != 1) {
                ...
                jQuery.each(this.instanceArray, function (instanceKey, instance) {
                    if (!jFormerUtility.empty(errorMessageArray[instanceKey])) {
                        jQuery.each(errorMessageArray[instanceKey], function (errorMessageArrayIndex, errorMessage) {
                            if (errorMessage != '') {
    ...
    handleErrors: function () {
        ...
        var tipErrorUl = jQuery('<ul id="' + this.id + '-tipErrorUl" class="tipErrorUl"></ul>');
        jQuery.each(this.errorMessageArray, function (index, errorMessage) {
    ...
    createTipDiv: function () {
        ...
        this.tipDiv = jQuery('<div id="' + this.id + '-tip" style="display: none;"></div>');
    ...
    disableByDependency: function (disable) {
        ...
        jQuery.each(this.instanceArray, function (index, componentInstance) {
            if (index !== 0) {
        ...
        if (this.options.instanceOptions !== null && (this.instanceArray.length < this.options.instanceOptions.max || this.options.instanceOptions.max === 0)) {
            ...
            if (self.parentJFormSection.parentJFormPage.jFormer.initializing) {
                if (!disable && addButton.is(':hidden')) {
                    addButton.show();
                    self.parentJFormSection.parentJFormPage.jFormer.adjustHeight({
                        adjustHeightDuration: 0
                    });
        ...
        if (self.parentJFormSection.parentJFormPage.jFormer.initializing) {
            ...
            animationOptions = jQuery.extend(animationOptions, this.parentJFormSection.parentJFormPage.jFormer.options.animationOptions.dependency, this.options.dependencyOptions.animationOptions);
        } else {
...
JFormComponentAddress = JFormComponent.extend({
    ...
    getValue: function () {
        ...
        this.component.find(':input').each(function (index, input) {
            address[jQuery(input).attr('id').replace(self.id + '-', '')] = jQuery(input).val();
        });
        if (this.options.emptyValue) {
    ...
    validate: function () {
        ...
        setTimeout(function () {
            if (!self.component.hasClass('jFormComponentHighlight')) {
                ...
                self.clearValidation();
                jQuery.each(self.options.validationOptions, function (validationType, validationOptions) {
    ...
    addEmptyValues: function () {
        ...
        jQuery.each(emptyValue, function (key, value) {
            var input = self.component.find('input[id*=' + key + ']');
            input.addClass('defaultValue');
            input.focus(function (event) {
                if (jQuery.trim(jQuery(event.target).val()) == value) {
                    jQuery(event.target).val('');
                    jQuery(event.target).removeClass('defaultValue');
                }
            });
            input.blur(function (event) {
                if (jQuery.trim(jQuery(event.target).val()) == '') {
                    jQuery(event.target).addClass('defaultValue');
...
JFormComponentCreditCard = JFormComponent.extend({
    ...
    addEmptyValues: function () {
        ...
        jQuery.each(emptyValues, function (key, value) {
            var input = self.component.find('input[id*=' + key + ']');
            input.addClass('defaultValue');
            input.focus(function (event) {
                if (jQuery.trim(jQuery(event.target).val()) == value) {
...
JFormComponentDate = JFormComponent.extend({
    ...
    setValue: function (value) {
        ...
        if (value == null || value.month == 'undefined' || value.year == 'undefined' || value.day == 'undefined') {
            ...
            jQuery('#' + this.id).val(padString(value.month) + '/' + padString(value.day) + "/" + value.year)
...
JFormComponentDropDown = JFormComponent.extend({
    ...
    getValue: function () {
        ...
        var dropDownValue = jQuery('#' + this.id).val();
        return dropDownValue;
    },

    setValue: function (value) {
        jQuery('#' + this.id).val(value).trigger('jFormComponent:changed');
    ...
    initialize: function () {
        ...
        this.validationFunctions = {
            ...
            'extensionType': function (options) {
                ...
                if (jQuery.isArray(options.extensionType)) {
...
JFormComponentLikert = JFormComponent.extend({
    ...
    initialize: function () {
        ...
        jQuery.each(this.options.statementArray, function (statementName, statementOptions) {
    ...
    clearValidation: function () {
        jQuery.each(this.statementComponentArray, function (index, statement) {
    ...
    getValue: function () {
        var value = {};
        jQuery.each(this.statementComponentArray, function (key, component) {
    ...
    handleServerValidationResponse: function (errorMessageArray) {
        ...
        if (errorMessageArray.length > 0) {
            jQuery.each(this.instanceArray, function (key, instance) {
                jQuery.each(errorMessageArray, function (index, passedErrorArray) {
                    jQuery.each(passedErrorArray, function (statementKey, statementError) {
...
JFormComponentLikertStatement = JFormComponent.extend({
    ...
    initialize: function () {
        var self = this
        this.changed = false;
        this.component = jQuery('input[name=' + this.id + ']:first').closest('tr');
        this.tipTarget = this.component;
        ...
        //event.preventDefault();
        //jQuery(event.target).find('input').attr("checked", "checked").trigger('click');
        //});
        this.validationFunctions = {
            'required': function (options) {
                var errorMessageArray = ['Required.'];
    ...
    addChoiceTips: function () {
        ...
        if (tips.length > 0) {
            tips.each(function (index, tip) {
                var tipTarget = jQuery(tip).prev('label').find('.jFormComponentMultipleChoiceTipIcon');
                if (tipTarget.length == 0) {
                    tipTarget = jQuery(tip).parent();
                }
                tipTarget.simpletip({
                    position: 'topRight',
                    content: jQuery(tip),
    ...
    getValue: function () {
        ...
        if (this.options.multipleChoiceType == 'checkbox') {
            ...
            this.component.find('input:checked').each(function (index, input) {
                multipleChoiceValue.push(jQuery(input).val());
            });
        } else {
...
JFormComponentSingleLineText = JFormComponent.extend({
    ...
    initialize: function () {
        ...
        this.validationFunctions = {
            ...
            'blank': function (options) {
                ...
                return jQuery.trim(options.value).length == 0 ? 'success' : errorMessageArray;
            },
            ...
            'matches': function (options) {
                ...
                return options.value == jQuery('#' + idToMatch).val() ? 'success' : errorMessageArray;
            },
            ...
            'postalZip': function (options) {
                var errorMessageArray = ['Must be a valid Indian zipcode.']
            ...
            'zip': function (options) {
                var errorMessageArray = ['Must be a valid Indian zipcode.'];
                return options.value == '' || options.value.match(/^[0-9]{6}(?:-[0-9]{5})?$/) ? 'success' : errorMessageArray;
            }
        }
    },
    ...
    setValue: function (value) {
        jQuery('#' + this.id).val(value).removeClass('defaultValue');
        this.validate(true);
    },
    ...
    addEmptyValue: function () {
        ...
        input.focus(function (event) {
            ...
            if (jQuery.trim(target.val()) == emptyValue) {
                target.val('');
...
JFormComponentTextArea = JFormComponent.extend({
    ...
    addAutoGrow: function () {
        ...
        var update = function () {
                ...
                var val = textArea.val().replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/&/g, '&amp;').replace(/\n$/, '<br/>&nbsp;').replace(/\n/g, '<br/>').replace(/ {2,}/g, function (space) {
        ...
        jQuery(textArea).change(update).keyup(update).keydown(update);
