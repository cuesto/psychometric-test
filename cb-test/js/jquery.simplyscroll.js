...
jQuery.simplyScroll = function(el,options) {
	...
	this.$list = jQuery(el); //called on ul/ol/div etc
	...
	if ($items.length > 1) {
		...
		if (this.isHorizontal) {
			$items.each(function() { total+=jQuery(this).outerWidth(true); });
			extra_wrap = $items.eq(0).outerWidth(true) * $items.length !== total;
		} else {
			$items.each(function() { total+=jQuery(this).outerHeight(true); });
...
jQuery.simplyScroll.fn.extend({
	init: function() {
		...
		this.$btnBack = jQuery('.simply-scroll-back',this.$container);
		this.$btnForward = jQuery('.simply-scroll-forward',this.$container);
		...
		if (this.isAuto) {
			...
			if (this.isAuto && this.o.pauseOnHover && !this.supportsTouch) {
				...
				if (this.o.pauseButton) {
					...
					this.$btnPause = jQuery(".simply-scroll-btn-pause",this.$container)
